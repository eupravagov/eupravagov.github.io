jQuery(document).ready(function () {
  jQuery("#confirmNotificationsUpdate").on("click", function (e) {
    ...
    var isSuccessessfullyUpdatedNotification = jQuery(
      "#IsSuccessessfullyUpdatedNotification"
    );
    ...
    for (var i = 0; i < checkboxes.length; i++) {
      ...
      var notificationChecked = document.querySelectorAll(
        "input[type=checkbox]"
      )[i].checked;
    ...
    function parseNotifcations(array) {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        const notification = groups.find(
          (g) => g.NotificationGroupId === notificationGroupId
        );
        //const notificationIndex = groups.findIndex(g => g.NotificationGroupId === notificationGroupId);

        if (notification) {
          notification.DeliveryTypes.push({
