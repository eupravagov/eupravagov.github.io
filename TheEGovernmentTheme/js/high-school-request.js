jQuery(document).ready(function () {
  jQuery("input[type=radio][name=MedicalExaminationSelection]").change(
    function () {
      if (this.value == "no") {
  ...
  $selects.on("change", function () {
    ...
    $selects.each(function () {
      ...
      $options.each(function () {
        if (jQuery(this).text() == selectedText && selectedText != "")
...
function CancelRequestExecute() {
  jQuery("#yes-confirm-message.form-popup").removeClass("visible");
  jQuery("#submitButton").prop("disabled", true);
  jQuery("input[type=radio][name=MedicalExaminationSelection]").prop(
    "checked",
    false
  );
  return false;
